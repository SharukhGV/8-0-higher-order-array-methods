...
function allSongsAreOverTwoMinutes(songs) {
  let result = songs.every((isOver)=> isOver.runtimeInSeconds>120);
  return result
}
...
function anySongIsOverFourMinutes(songs) {
  let result = songs.some((isOver)=> isOver.runtimeInSeconds>240);
  return result
}
...
function anySongIsByPeanut(songs) {
  ...
  let result = songs.some((isOver)=> isOver.artist==="Peanut");
  return result
