...
function getSongsBySaib(songs) {
  ...
  let result = songs.filter(function (arrayArt){return arrayArt.artist === 'Saib'} )
  return result
}
...
function getSongsOverThreeMinutes(songs) {
  ...
  let result = songs.filter(function(timeYeah){ return timeYeah.runtimeInSeconds >180})
  return result
}
...
function getTitleTracks(songs) {
  let result = songs.filter(function(samething){return samething.title === samething.album})
  return result
}
